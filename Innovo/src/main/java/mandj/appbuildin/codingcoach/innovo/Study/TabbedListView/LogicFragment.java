 ...
 * interface.
 */
public class LogicFragment extends ListFragment {

    //static final String[] list = new String[]{"addTwo", "lessThanFour","twoOrThree","Locked","Locked"};
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        stuff.setupLogic(getActivity().getApplicationContext());
        rowItems = new ArrayList<RowItem>();
        for (int i = 0; i < stuff.getLogicDifficulty().length; i++) {
            RowItem item = new RowItem(stuff.getLogicDifficulty()[i], stuff.getLogicTitles()[i], stuff.getLogicDescripts()[i], stuff.getLogicDone()[i]);
            rowItems.add(item);
        }
    ...
    public void onListItemClick(ListView l, View v, int position, long id) {
        ...
        String[] names = myContext.getResources().getStringArray(R.array.method_header_logic);
        if (position < names.length) { //if position less than number of puzzles
            ...
            intent.putExtra("difficulty", stuff.getLogicDifficulty()[position]);
