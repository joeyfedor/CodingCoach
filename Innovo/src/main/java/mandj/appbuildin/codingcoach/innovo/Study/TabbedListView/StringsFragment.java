...
public class StringsFragment extends ListFragment {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        stuff.setupStrings(getActivity().getApplicationContext());
        rowItems = new ArrayList<>();
        for (int i = 0; i < stuff.getStringsDifficulty().length; i++) {
            RowItem item = new RowItem(stuff.getStringsDifficulty()[i], stuff.getStringsTitles()[i], stuff.getStringsDescripts()[i], stuff.getStringsDone()[i]);
            rowItems.add(item);
        }
    ...
    public void onListItemClick(ListView l, View v, int position, long id) {
        ...
        String[] names = myContext.getResources().getStringArray(R.array.method_header_strings);
        if (position < names.length) { //if position less than number of puzzles
            ...
            intent.putExtra("difficulty", stuff.getStringsDifficulty()[position]);
